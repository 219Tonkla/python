## การเขียนโปรแกรม  ctrl n เปิดหน้าต่างใหม่ กดshift alt ลง คือการคัดลอกวางซ้ำๆ
## ctrl s เซฟไฟล์ 
## ctrl+d เปิดexplorer
## ctrl+j เปิดterminal
## ctrl shift n เปิดvsโค้ดใหม่
## ctrl shift k เป็นการลบที่ละบรรทัด 
## alt ค้าง แล้วสลับตำแหน่งด้วยลูกศรขึ้นลง##
## shift alt ลากลงมา จะพิมพ์ได้เหมือนกันทุกบรรทัด##
## ctrl d กดลูกศรซ้ายขวา จะพิมพ์ได้เหมือนกันทุกบรรทัด
## esc 
## ctel f2 คลุมตัวที่เราต้องการเลือกทั้งหมด
## หาไฟล์แบบเร็ว กดctel p
## เปิดไฟล์อีกไฟล์ กดctrl และกดตรง ฃ
## ctel w ปิดเปิดไฟล์
## ctrl shift x 
## run พิมพ์python ตามด้วยชื่อไฟล์
## กดที่ไฟล์ กดf2 รีเนมไฟล์
## dirve c 
## mkdir foder 
## ctrl j 
## c alt f4
##x2 = 15 % 4
#x3 = 2 ** 4
#x4 = 100 // 9
#x5 = 100 // 9
#print(x1) ## mod การหารเอาเศษเหลือ
#print(x2) ## การยกกำลัง
#print(x3) ## หารโดยไม่เอาเศษ หรือปัดเศษทิ้ง
# เครื่องหมายor = l & จริงเป็นจริง จริงเท็จเป็นจริง เท็จเท็จเป็นเท็จ
print ('hello woelds')
print ('hello woelds')
print ('hello woelds')
print ('hello woelds')
print ('hello woelds')
print ('hello woelds')
print ('hello woelds')
print ('hello woelds')
print ('hello woelds')
print ('hello woelds')
print ('hello woelds')
print ('hello woelds')   
print ('hello woelds11')